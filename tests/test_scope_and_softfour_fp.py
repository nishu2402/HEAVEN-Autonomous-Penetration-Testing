...
def test_url_seed_does_not_authorise_subdomains():
    ...
    g = ScopeGuard({"urls": ["https://example.co.uk"]})
    assert g.allows("example.co.uk")                 # the exact seed host
    assert not g.allows("www.example.co.uk")         # a subdomain — NOT authorised
    assert not g.allows("autodiscover.example.co.uk")  # third-party (M365) infra
    assert not g.allows("remote.example.co.uk")
...
def test_feedback_drops_out_of_scope_subdomains_from_url_seed():
    ...
    eng = FeedbackEngine({"urls": ["https://example.co.uk"]})
    eng.ingest_result({"subdomains": [
        {"host": "www.example.co.uk"},
        {"host": "autodiscover.example.co.uk"},
        {"host": "remote.example.co.uk"},
    ]})
    host_leads = [ld.value for ld in eng.drain() if ld.kind == HOST]
    assert host_leads == []            # nothing actively scanned
    assert "www.example.co.uk" in eng.out_of_scope_hosts
    assert "autodiscover.example.co.uk" in eng.out_of_scope_hosts
...
def test_allows_subdomains_only_for_deliberate_domain_scope():
    ...
    url_seed = ScopeGuard({"urls": ["https://example.co.uk"]})
    assert not url_seed.allows_subdomains("example.co.uk")
    assert not url_seed.allows_subdomains("https://example.co.uk")
